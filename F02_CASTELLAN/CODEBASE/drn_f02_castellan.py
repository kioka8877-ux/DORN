...
HUD de contrôle et simulation. L'opérateur visualise l'animation, corrige
le plan_de_vol.json en temps réel, puis fige le plan de vol pour F03.
...
Entrées  : F02_CASTELLAN/IN/plan_de_vol.json + F02_CASTELLAN/IN/images/*.png
...
<script>
const C=JSON.parse('{cfg.replace(chr(39), chr(92)+chr(39))}');
...
function draw(progress){{
  ...
  else{{const[mn,mx]=yRange(curves,xCur);drawGrid(mn,mx);curves.forEach(c=>drawCartCurve(c,xCur,mn,mx));}}
  if(frozen)drawFreezeOverlay();
  inf.textContent=`x=${{xCur.toFixed(3)}} | t=${{tSec}}s | frame ${{frame}}/${{totF}}${{frozen?' [FREEZE]':''}}`;}}
...
def main():
    ...
    with col_sim:
        ...
        st.caption("Déplacer le slider pour parcourir l'animation. Le rendu final apparaît à l'extrémité droite.")
        canvas_height = 560 if cm.get("format") == "vertical" else 420
        ...
        if data.get("validated_by_magos"):
            ...
            st.info("Vérifier l'animation ci-dessus, corriger si besoin, puis FIGER.")
